...
class LocLoader():
    ...
    def location_loader(self):
        ...
        try:      
            finnish_communities="https://fi.wikipedia.org/wiki/Luettelo_Suomen_kuntien_koordinaateista#Nykyiset_kunnat"
            communities=pd.read_html(requests.get(finnish_communities).content)[2]
            communities[["Latitude","Longitude"]]=communities["Koordinaatit"].str.split("°N, ",expand=True)
            ...
            communities[["Latitude","Longitude"]]=communities[["Latitude","Longitude"]].apply(pd.to_numeric, errors='coerce', axis=1)
